...
#' @param new_param all the required details for the new parameter...
...
#' @examples             Note that this is a much shorter example as R can't handle full length parameter structure
#' create_new_parameter_entry <- function(pathogen  = NA,
                                      ...
                                      vignette_prepend = "")
{
  ...
  old_parameters       <- as_tibble(load_data(table_type = 'parameter',pathogen = pathogen, vignette_prepend = vignette_prepend))
  ...
  new_row              <- new_row %>% dplyr::select(colnames(old_parameters))
  ...
  file_path_ob  <- system.file("data", "access_db_dropdown_parameters.csv", package = "epireview")
  if(file_path_ob=="") file_path_ob <- paste0(vignette_prepend,"data/access_db_dropdown_parameters.csv")
  ...
  rules <- validator(
    ...
    model_types_valid               = strsplit(population_country,",")[[1]] %vin% na.omit(parameter_options$`Population Country`)
  )
