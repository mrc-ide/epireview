...
quality_assessment_plots <- function(location_prepend='',pathogen='marburg')
{
  ...
  QA_time_series <- quality %>% filter(!is.na(year_publication) & !is.na(pathogen)) %>%
    ...
    scale_x_continuous(breaks = seq(round(min(quality$year_publication,na.rm = TRUE),-1), # nearest decade to first publication
                                    as.double(substring(Sys.time(),1,4)),                 # current year
                                    by=10))
